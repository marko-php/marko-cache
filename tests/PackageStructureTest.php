...
it('has a valid composer.json with correct package name marko/cache', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';

    expect(file_exists($composerPath))->toBeTrue()
        ->and(json_decode(file_get_contents($composerPath), true))->toBeArray()
        ->and(json_decode(file_get_contents($composerPath), true)['name'])->toBe('marko/cache');
});
...
it('has correct description in composer.json', function () {
    ...
    expect($composer['description'])->toBe('Cache interfaces and infrastructure for Marko Framework');
});
...
it('has PSR-4 autoloading configured for Marko\Cache namespace', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode(file_get_contents($composerPath), true);

    expect($composer)->toHaveKey('autoload')
        ->and($composer['autoload'])->toHaveKey('psr-4')
        ->and($composer['autoload']['psr-4'])->toHaveKey('Marko\\Cache\\')
        ->and($composer['autoload']['psr-4']['Marko\\Cache\\'])->toBe('src/');
});
...
it('has default cache.php config file', function () {
    $configPath = dirname(__DIR__) . '/config/cache.php';
    ...
    expect($config)->toBeArray()
        ...
        ->and($config)->toHaveKey('default_ttl');
});
